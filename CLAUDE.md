...
| **Lentitud/fallos al cargar — dos causas de raíz encontradas y medidas con Playwright (v4.22.3, 07/07/2026).** (1) `getMyCoupleId()` llamaba a `supabase.auth.getUser()` (siempre pega al servidor de Auth) para obtener `user.id`, DUPLICANDO la verificación de sesión que `AppWithAuth` ya había hecho un instante antes vía `getSession()` (mayormente local) — un round-trip de red entero, evitable, en cada carga en frío. (2) El guardado de migración (`saveData` tras `applyCarryOver`/repairs) se esperaba con `await` antes de `setLoading(false)` — hasta 15s de skeleton de más en cualquier carga que migre algo (todos los lunes, por el carry-over automático) — contradiciendo el propio comentario del código ("no bloquear el arranque"). Medido: con save de fondo artificialmente lento (6s), el skeleton tardaba 6.6s en desaparecer; tras el fix, 0.5s. | **Regla de auditoría de latencia**: ante un reporte de "tarda en cargar", contar los round-trips de red SECUENCIALES en la cadena crítica (`getSession → getMyCoupleId → loadData → repairs`) y buscar (a) verificaciones de sesión duplicadas — pasar `user.id`/`session` ya resuelto en vez de re-verificar, y (b) cualquier `await` de un save/efecto "best-effort" que se ejecute ANTES de liberar el loading/skeleton — si el comentario dice "no bloquear", el código no debe tener `await` ahí. Verificar con una prueba que mockee la llamada lenta con un delay deliberado y mida el tiempo real hasta que el contenido aparece, no solo "funciona". |
| **El problema de carga persistía incluso con la PWA correctamente instalada en iPhone (ícono de pantalla de inicio, no pestaña de Safari) — confirmado por el usuario, descartando "mal uso" (v4.22.4, 07/07/2026).** WKWebView puede colgar el PRIMER `fetch()` tras un cold start/volver de background para SIEMPRE (ni resuelve ni rechaza), incluso en apps instaladas correctamente — el `withTimeout` existente (v4.20.2) detecta el cuelgue y falla, pero antes de este fix la única salida era mostrar un error, no recuperarse. | **Regla de resiliencia de red para iOS**: cuando el fallo de red es un CUELGUE (no un error real del servidor), la estrategia correcta no es solo "detectar y fallar rápido" — es "detectar, reintentar UNA vez con una request nueva, y solo entonces fallar". Nueva utilidad `withTimeoutRetry(fn, ms, label, retries=1)` en `utils.js`: `fn` debe ser una FACTORY (`() => promise`), nunca un promise ya creado, porque reintentar exige una request nueva (el promise colgado original nunca se resuelve). Aplicada a las 3 llamadas críticas del arranque (`getSession`, `getMyCoupleId`, `loadData`/`loadFromNormalized`) — no a los paths de guardado, donde reintentar automáticamente una escritura tiene implicaciones de idempotencia distintas y requiere su propio análisis. |
| **El mismo cuelgue de WKWebView apareció en el GUARDADO, no solo en la carga (v4.22.5, 07/07/2026).** Tras desplegar v4.22.4, el usuario vio `timeout: saveWithCAS tras 15000ms` con la app ya abierta y funcionando — v4.22.4 dejó el path de guardado explícitamente afuera del retry automático hasta analizar si reintentar una escritura era seguro. | **El análisis de seguridad**: `saveWithCAS` usa Compare-And-Swap (`save_app_data_cas` RPC con `p_version`) — reintentar es seguro porque si el intento colgado en realidad ya escribió en el servidor, el reintento con la versión vieja simplemente encuentra un mismatch (`conflict`) y dispara el rebase-on-conflict que YA existe para cuando la pareja guarda primero. Nunca duplica ni pierde nada. **Regla general para decidir si una escritura es segura de reintentar tras un cuelgue**: ¿el reintento, si el original en realidad tuvo éxito, produce un resultado detectable (version mismatch, constraint violation, respuesta idéntica) en vez de un efecto secundario duplicado (INSERT sin idempotencia, incremento, envío de notificación)? Si sí, aplicar `withTimeoutRetry` igual que en las lecturas. `saveWithCAS`, `saveWithRetry` (fallback, upsert idempotente con el mismo blob) y sus recargas de versión asociadas ya usan el wrapper. |
| **El banner de error de guardado mostraba el texto crudo de la excepción (`timeout: saveWithCAS tras 15000ms`) cuando el reintento automático (v4.22.5) también fallaba (v4.22.6, 07/07/2026).** Técnicamente correcto, pero alarmante y sin contexto para quien no es desarrollador — no explica que el cambio sigue guardado localmente ni que la app va a reintentar sola. | **Regla de mensajes de error para el usuario final**: el texto de una excepción (`e.message`) es para logs/analítica (`console.warn` + `track()`), nunca para mostrarse crudo en la UI. Todo `catch` que le muestre algo al usuario debe traducir el error a qué significa PARA ÉL (¿se perdió algo? ¿qué va a pasar ahora?), no a qué falló técnicamente. Aplica en particular a los `catch` de `runSave` — ya tiene el hábito correcto en otras partes (`setError` en la carga inicial es un mensaje propio, no `e.message`); esta fila lo alinea también en el guardado. |
| **CAUSA RAÍZ del "error al guardar" recurrente — blob de 4MB por fotos base64 (v5.14.0, 21/08/2026, diagnosticado con acceso directo a Supabase).** El "error al guardar" NO era red ni CAS: era **`statement_timeout` del rol `authenticated` = 8s** contra un blob de **4MB** (3.8MB eran fotos de semana en base64 dentro de `weeks`, una por semana, >1 año acumulado). Cada guardado reescribía los 4MB **y** el trigger `backup_app_data` los copiaba otra vez a `app_data_backups` → bajo carga se pasaba de 8s → Postgres cancelaba la escritura (logs: **65 `canceling statement due to statement timeout`/24h**). La versión avanzaba cuando sí entraba a tiempo (por eso "a veces guarda"). En iOS peor porque 4MB revientan la cuota de localStorage. Los fixes v5.13.0/v5.13.1 (fallback CAS + diagnóstico) no tocaban la causa. | **(1) Alivio inmediato server-side:** `ALTER ROLE authenticated SET statement_timeout='20s'` (los guardados de 4MB entran con margen; reversible). **(2) Cura de raíz client-side:** fotos de semana FUERA del blob → bucket público `photos` de Storage (`src/lib/photoStore.js`); el blob guarda solo `week.photoUrl`. Migración automática al cargar (`isInlinePhoto` → `uploadWeekPhoto` → `update`), blob de 4MB → ~200KB. SW cachea el bucket (`week-photos`, CacheFirst) para offline. **Regla: NADA de imágenes/binarios en base64 dentro del blob `app_data.data` — van a Storage y el blob solo referencia la URL. Antes de dar por bueno un guardado lento, medir `pg_column_size(data)` y el `statement_timeout` del rol `authenticated`, no asumir que es la red.** |
| **"Error al guardar" recurrente en AMBAS plataformas (iOS y Android) (v5.13.0, 21/08/2026).** El bucle CAS de `runSave` reintentaba hasta 6 veces ante `conflict` (RPC sin fila devuelta) y, si no convergía, hacía `throw` → banner de error. Problema: si el RPC `save_app_data_cas` devuelve vacío en un guardado EXITOSO (p.ej. RLS filtra la fila del `RETURNING`, o alta contención real entre la pareja), CADA guardado parece un conflicto → 6 rebases → error, en las dos plataformas, aunque el dato SÍ se escriba. El `throw` no intentaba el fallback seguro. Diagnóstico a ciegas (sin acceso a Supabase en la sesión): el historial confirma que los bugs de CAS han sido siempre del cliente. | **Degradación con gracia**: el bucle CAS baja de 6 a 3 intentos (no encadenar timeouts largos en iOS) y, si no converge, en vez de `throw` hace un **last-write-wins seguro** (`saveWithRetry`) del `toSave` ya rebasado sobre los datos frescos del último reload — sube el estado FUSIONADO, no pisa a la pareja. Convierte el "error" recurrente en guardado correcto. Se loguea `cas_rpc_error` (mensaje real del RPC) y `cas_no_converge_fallback` para diagnóstico. **Regla: ningún path de guardado puede terminar en "error" mientras exista un fallback idempotente seguro no agotado — el error solo es legítimo cuando la red está realmente caída tras agotar reintentos.** |
| **Carga/uso con MALA SEÑAL caía a la pantalla de error y la app no se podía usar offline (v5.12.0, 20/08/2026).** La copia local (fast-path del arranque + fallback offline) vivía SOLO en `localStorage` (~5MB, síncrono). Con fotos en base64 en el blob, `saveLocalBackup` superaba la cuota y lanzaba `QuotaExceededError` (tragado en silencio) → sin copia local. Además iOS desaloja `localStorage` por presión/ITP. Y el path de blob (`loadDataWithVersion`, el activo) NO escribía backup local en la carga — solo `saveData`/realtime — así que quien solo abría y cerraba nunca dejaba copia. Resultado: cuando el fast-path no tenía datos, una carga con señal débil (donde `navigator.onLine` sigue `true` y el `fetch` cuelga hasta el timeout) agotaba ~20s de skeleton y caía a la pantalla de error de integridad, sin modo offline usable. | **Persistencia local DURABLE en IndexedDB** (`src/lib/localStore.js`, ~cientos de MB, sobrevive mejor a la evicción): `saveLocalBackup` escribe en IDB (durable, best-effort) **y** localStorage (caché rápida síncrona); `loadLocalBackupAsync` lee la más reciente de ambas (`pickFreshest`, puro + tests) y repuebla localStorage si se perdió. El arranque usa la versión async. **Regla: cualquier copia local de la que dependa el arranque/offline va en IndexedDB, no solo en localStorage** — localStorage revienta por cuota con blobs grandes y lo desaloja iOS. Además: **toda carga exitosa persiste la copia durable** (`saveLocalBackup(base)` tras `setData`), no solo los guardados — si no, quien solo abre y cierra se queda sin copia. Y **offline explícito** (`!navigator.onLine`) hace short-circuit: con copia local sigue con ella, sin copia muestra pantalla "sin conexión" amable (no la de integridad) y auto-recarga al volver la señal. |
| **`isValidAppData` solo valida FORMA, no magnitud — un blob estructuralmente válido con el 90% de las misiones borradas (o vacío) pasaba todos los gates y se guardaba en silencio (v4.25.0, 13/07/2026).** Detectado al blindar la app tras el episodio del dashboard vacío: ningún check impedía que una respuesta incompleta de una sync o un import chico sobrescribieran datos buenos. Además, los backups server-side (`app_data_backups`, poblados por `trg_snapshot_app_data` en cada UPDATE) existían desde el 26/05 pero la app NUNCA los consultaba — recuperación solo por SQL manual. | **Write-guard (`src/lib/dataGuards.js`, funciones puras + 18 tests)**: `assessWrite(prevConfirmado, next)` bloquea en `runSave` ANTES de red y de `saveLocalBackup` si el nuevo estado conserva <60% de las misiones (o queda en 0 habiendo ≥3) — `ConfirmDialog` con números concretos; "recuperar" descarta y recarga del servidor. El flush de background (que bypassea `runSave`) lleva el mismo check. `lastConfirmedCountRef` se actualiza en TODO punto de estado confirmado (carga, realtime, save OK, smartSync, forcePush, re-fetch de foreground) — olvidarlo en un punto nuevo produce falsos positivos. **Fallback de backups**: si la carga falla sin copia local, se ofrece (nunca auto-aplica) el último `app_data_backups` que pase `isBackupUsable()`. Reglas: **(1)** cualquier gate nuevo de guardado valida magnitud además de forma; **(2)** una restauración es una escritura — siempre requiere confirmación explícita; **(3)** en cargas, las discrepancias grandes AVISAN pero no bloquean (pueden ser borrados legítimos de la pareja) — el bloqueo duro va solo en la escritura. |
