...
export default function GastosView({ gastos, proyectos, p1, p2, colors, onUpdate, onUpdateProyectos, onUpdateAll }) {
  ...
  const TAB_BTN = (active) => ({ flexShrink:0, padding:"6px 14px", borderRadius:20, border:"none", cursor:"pointer", fontFamily:"inherit", fontSize:12, fontWeight:active?600:400, background:active?"var(--t-accent-soft,rgba(167,139,250,0.2))":"rgba(128,128,128,0.07)", color:active?"var(--t-accent,#a78bfa)":"var(--t-text-dim,#8f84ad)" });
  ...
  return (
    <div style={{ padding:"16px 16px 80px", maxWidth:600, margin:"0 auto" }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:16, overflowX:"auto", paddingBottom:2 }}>
        ...
        <button onClick={openAddProject} style={{ flexShrink:0, padding:"6px 12px", borderRadius:20, border:"1px dashed rgba(167,139,250,0.3)", cursor:"pointer", fontFamily:"inherit", fontSize:12, background:"transparent", color:"var(--t-text-dim,#8f84ad)" }}>+ Proyecto</button>
        ...
        return (
          <div style={{ ...S.card, marginBottom:14, display:"flex", alignItems:"center", gap:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginTop:1 }}>{gastos.filter(g=>g.projectId===projectId).length} gastos · {fmtAmt(pb.total)}</div>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:4, alignItems:"flex-end" }}>
              ...
              <div style={{ display:"flex", gap:4 }}>
                <button onClick={()=>openEditProject(p)} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", cursor:"pointer", fontSize:13, padding:"2px 5px" }} onMouseEnter={e=>e.currentTarget.style.color="#c4b8ff"} onMouseLeave={e=>e.currentTarget.style.color="#6b5f88"}>✎</button>
                <button onClick={()=>confirm(`¿Eliminar proyecto "${p.name}"?`, ()=>delProject(projectId))} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", cursor:"pointer", fontSize:17, padding:"2px 5px" }} onMouseEnter={e=>e.currentTarget.style.color="#f472b6"} onMouseLeave={e=>e.currentTarget.style.color="#3d3360"}>×</button>
              </div>
            </div>
          </div>
        );
        ...
        const renderCard = p => (
          <button key={p.id} onClick={()=>{setProjectId(p.id);setView("list");}}
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)" }}>{p.count} gastos · {fmtAmt(p.total)}</div>
            </div>
            ...
            <span style={{ color:"var(--t-text-dim,#8f84ad)" }}>›</span>
          </button>
        );
        if (!open.length && !closed.length) return (
          <div style={{ textAlign:"center", padding:40, color:"var(--t-text-dim,#8f84ad)", marginBottom:14 }}>
            ...
            <button onClick={openAddProject} style={{ marginTop:14, padding:"8px 16px", borderRadius:20, border:"1px dashed rgba(167,139,250,0.3)", cursor:"pointer", fontFamily:"inherit", fontSize:12, background:"transparent", color:"var(--t-text-dim,#8f84ad)" }}>+ Proyecto</button>
          </div>
        );
        return (
          <div style={{ marginBottom:14 }}>
            {open.length>0 && <>
              <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:8 }}>📁 Proyectos activos</div>
              {open.map(renderCard)}
            </>}
            {closed.length>0 && <>
              <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:8, marginTop:open.length?14:0 }}>🔒 Saldados</div>
        ...
        return (
          <>
            <div style={{ ...S.card, marginBottom:14 }}>
              <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:14 }}>📅 Últimos 6 meses</div>
              <div style={{ display:"flex", alignItems:"flex-end", gap:4, height:90 }}>
                {monthlyTotals.map(({m,total})=>(
                  <div key={m} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3 }}>
                    <div style={{ fontSize:9, color:"var(--t-text-dim,#8f84ad)", height:14, textAlign:"center" }}>{total>0?fmtAmt(total):""}</div>
                    ...
                    <div style={{ fontSize:9, color:"var(--t-text-dim,#8f84ad)" }}>{monthLabel(m).slice(0,3)}</div>
                  </div>
                ))}
              </div>
            </div>
            {allCats.length>0&&(
              <div style={{ ...S.card, marginBottom:14 }}>
                <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:12 }}>🏷️ Por categoría (total)</div>
                {allCats.map(c=>(
                  <div key={c.id} style={{ marginBottom:9 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:3 }}>
                      <span style={{ fontSize:12, color:c.color, fontWeight:600 }}>{c.icon} {c.label}</span>
                      <span style={{ fontSize:12, color:"var(--t-text-muted,#b9b0d0)" }}>{fmtAmt(c.total)} · {allBal.total>0?Math.round(c.total/allBal.total*100):0}%</span>
                    </div>
            ...
            <div style={{ ...S.card, marginBottom:14 }}>
              <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:12 }}>👥 Resumen</div>
              <div style={{ display:"flex", gap:10, marginBottom:10 }}>
                <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:10, padding:"10px 12px" }}>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{p1} pagó</div>
                  <div style={{ fontSize:17, fontWeight:700, color:colors.person1||"#f472b6" }}>{fmtAmt(allBal.p1Paid)}</div>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{allBal.total>0?Math.round(allBal.p1Paid/allBal.total*100):0}% del total</div>
                </div>
                <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:10, padding:"10px 12px" }}>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{p2} pagó</div>
                  <div style={{ fontSize:17, fontWeight:700, color:colors.person2||"#60a5fa" }}>{fmtAmt(allBal.p2Paid)}</div>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{allBal.total>0?Math.round(allBal.p2Paid/allBal.total*100):0}% del total</div>
                </div>
              </div>
              <div style={{ display:"flex", gap:10, marginBottom:Math.abs(allBal.p1Net)>0.5?10:0 }}>
                <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:10, padding:"10px 12px" }}>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>Total acumulado</div>
                  <div style={{ fontSize:17, fontWeight:700, color:"var(--t-accent,#c4b8ff)" }}>{fmtAmt(allBal.total)}</div>
                </div>
                <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:10, padding:"10px 12px" }}>
                  <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>Promedio/mes</div>
            ...
            {proyectosWithBal.filter(p=>p.total>0).length>0&&(
              <div style={{ ...S.card }}>
                <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:12 }}>📁 Por proyecto</div>
                {proyectosWithBal.filter(p=>p.total>0).map(p=>(
                  <div key={p.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:"1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)" }}>{p.count} gastos</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
        ...
        <>
          <div style={{ ...S.card, marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
              <span style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600 }}>💸 {monthLabel(filterMonth)}</span>
          ...
          {catTotals.length>0&&(
            <div style={{ ...S.card, marginBottom:12 }}>
              <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:10 }}>🏷️ Por categoría</div>
              {catTotals.map(c=>(
                <div key={c.id} style={{ marginBottom:7 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:2 }}>
                    <span style={{ fontSize:12, color:c.color, fontWeight:600 }}>{c.icon} {c.label}</span>
                    <span style={{ fontSize:12, color:"var(--t-text-muted,#b9b0d0)" }}>{fmtAmt(c.total)} · {monthBalance.total>0?Math.round(c.total/monthBalance.total*100):0}%</span>
                  </div>
          ...
          {filteredList.length===0&&(
            <div style={{ textAlign:"center", padding:40, color:"var(--t-text-dim,#8f84ad)" }}>
              ...
              <div key={g.id} style={{ ...S.card, marginBottom:6, display:"flex", alignItems:"center", gap:10, padding:"10px 12px" }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:5, marginBottom:2, flexWrap:"wrap" }}>
                    ...
                    {projName&&<span style={{ fontSize:9, color:"var(--t-text-muted,#b9b0d0)", background:"rgba(128,128,128,0.10)", borderRadius:4, padding:"1px 4px", flexShrink:0 }}>📁 {projName}</span>}
                  </div>
                  <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)" }}>{g.date} · {paidLabel} pagó · {splitLabel}</div>
                </div>
                <div style={{ fontSize:15, fontWeight:700, color:"var(--t-accent,#c4b8ff)", flexShrink:0 }}>{fmtAmt(g.amount)}</div>
                <div style={{ display:"flex", flexDirection:"column", gap:2, flexShrink:0 }}>
                  <button onClick={()=>openEditExpense(g)} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", cursor:"pointer", fontSize:12, padding:"2px 4px" }} onMouseEnter={e=>e.currentTarget.style.color="#c4b8ff"} onMouseLeave={e=>e.currentTarget.style.color="#6b5f88"}>✎</button>
                  <button onClick={()=>delExpense(g.id)} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", cursor:"pointer", fontSize:15, padding:"2px 4px" }} onMouseEnter={e=>e.currentTarget.style.color="#f472b6"} onMouseLeave={e=>e.currentTarget.style.color="#3d3360"}>×</button>
                </div>
              </div>
            );
          })}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
          ...
          <div style={{ background:"var(--t-card,#1d1733)", borderRadius:"18px 18px 0 0", width:"100%", maxWidth:560, padding:"20px 20px 40px", display:"flex", flexDirection:"column", gap:12, maxHeight:"92vh", overflowY:"auto" }}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button onClick={()=>{setShowForm(false);setEditId(null);}} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", fontSize:22, cursor:"pointer" }}>×</button>
            </div>
            <input placeholder="Descripción (ej. Cena, Supermercado…)" value={form.desc} onChange={e=>setForm(f=>({...f,desc:e.target.value}))} style={INP} />
            <div>
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Monto</div>
            ...
            <div>
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Fecha</div>
              <input type="date" value={form.date} onChange={e=>setForm(f=>({...f,date:e.target.value}))} style={{ ...INP, colorScheme:"dark" }} />
            </div>
            <div style={{ display:"flex", gap:8 }}>
              <div style={{ flex:1 }}>
                <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Categoría</div>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>¿Quién pagó?</div>
            ...
            <div>
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:6 }}>División del gasto</div>
              {form.amount && parseFloat(form.amount)>0 && (
                <div style={{ display:"flex", gap:8, marginBottom:8 }}>
                  <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                    <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{p1} paga</div>
                    <div style={{ fontSize:15, fontWeight:700, color:"var(--t-accent,#c4b8ff)" }}>{fmtAmt(parseFloat(form.amount)*form.splitP1/100)}</div>
                    <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{form.splitP1}%</div>
                  </div>
                  <div style={{ flex:1, background:"rgba(128,128,128,0.06)", borderRadius:8, padding:"8px 10px", textAlign:"center" }}>
                    <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{p2} paga</div>
                    <div style={{ fontSize:15, fontWeight:700, color:"var(--t-accent,#c4b8ff)" }}>{fmtAmt(parseFloat(form.amount)*(100-form.splitP1)/100)}</div>
                    <div style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)" }}>{100-form.splitP1}%</div>
                  </div>
                </div>
              )}
              ...
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <span style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)", flexShrink:0 }}>0%</span>
                ...
                <span style={{ fontSize:10, color:"var(--t-text-dim,#8f84ad)", flexShrink:0 }}>100%</span>
              </div>
            </div>
            {(proyectos||[]).length>0&&(
              <div>
                <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Proyecto (opcional)</div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.75)", zIndex:200, display:"flex", alignItems:"flex-end", justifyContent:"center" }}
          ...
          <div style={{ background:"var(--t-card,#1d1733)", borderRadius:"18px 18px 0 0", width:"100%", maxWidth:560, padding:"20px 20px 40px", display:"flex", flexDirection:"column", gap:14 }}
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button onClick={()=>{setShowProjectForm(false);setEditProjectId(null);}} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", fontSize:22, cursor:"pointer" }}>×</button>
            </div>
            <div>
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Nombre</div>
            ...
            <div>
              <div style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginBottom:8 }}>Emoji</div>
