...
// así no interfiere con el scroll normal.
const THRESHOLD = 64;
...
export default function PullToRefresh({ onRefresh, refreshing, children }) {
  ...
  const onTouchMove = e => {
    ...
    if (dy <= 0 || window.scrollY > 2) { if (pull !== 0) setPull(0); return; }
    setPull(Math.min(dy * 0.38, 90)); // resistencia progresiva
  };
  const onTouchEnd = () => {
    if (pull >= THRESHOLD && !firedRef.current) { firedRef.current = true; onRefresh?.(); }
    setPull(0);
