    ...
    <div style={{ ...S.card, borderColor:met?`${clr.together}50`:"rgba(167,139,250,0.12)", transition:"border-color 0.3s" }}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:5, marginTop:3, flexWrap:"wrap" }}>
              ...
              <span style={{ fontSize:11, color:"var(--t-text-muted,#b9b0d0)" }}>{PERIOD_EMOJI[goal.period]} {PERIOD_LABEL[goal.period]} · {isMax?"máx.":"mín."} {goal.target}×</span>
            </div>
          </div>
        </div>
        <div style={{ display:"flex", gap:2 }}>
          <button onClick={onEdit} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--t-text-dim,#8f84ad)", fontSize:15, padding:"3px 5px" }}
            onMouseEnter={e=>e.currentTarget.style.color="#a78bfa"} onMouseLeave={e=>e.currentTarget.style.color="#4a4166"}>✏️</button>
          <button onClick={onArchive} title="Archivar" style={{ background:"none", border:"none", cursor:"pointer", color:"var(--t-text-dim,#8f84ad)", fontSize:13, padding:"3px 5px" }}
      ...
      {goal.deadline && (() => {
        ...
        return <div style={{ marginBottom:10, display:"flex", alignItems:"center", gap:6, background:expired?"rgba(244,114,182,0.1)":under24h?"rgba(244,114,182,0.08)":urgent?"rgba(251,146,60,0.08)":"rgba(167,139,250,0.06)", border:`1px solid ${expired||under24h?"rgba(244,114,182,0.3)":urgent?"rgba(251,146,60,0.25)":"rgba(167,139,250,0.15)"}`, borderRadius:8, padding:"6px 10px" }}>
          ...
          <span style={{ fontSize:11, color:"var(--t-text-dim,#8f84ad)", marginLeft:"auto" }}>{goal.deadline}</span>
        </div>;
      })()}
      ...
      {history.length > 0 && (
        <div>
          <div style={{ fontSize:9, letterSpacing:1.5, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", marginBottom:5 }}>Historial</div>
            ...
            {history.map((h, i) => {
              ...
              return <button key={i}
                ...
                style={{ minWidth:28, height:28, borderRadius:7, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontSize:10, gap:1,
                  ...
                  color:selected?"#c4b8ff":noData?"var(--t-text-dim,#8f84ad)":failed?"#f472b6":h.met?"#34d399":"var(--t-text-dim,#8f84ad)",
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      {!active.length && !showForm && (
        <div style={{ textAlign:"center", padding:48, color:"var(--t-text-dim,#8f84ad)" }}>
      ...
      {archived.length > 0 && (
        <div>
          <div style={{ fontSize:10, letterSpacing:2, textTransform:"uppercase", color:"var(--t-text-dim,#8f84ad)", fontWeight:600, marginBottom:8, marginTop:4 }}>Archivadas</div>
          {archived.map(g => (
            <div key={g.id} style={{ ...S.card, opacity:0.5, marginBottom:6, display:"flex", alignItems:"center", gap:10, padding:"10px 14px" }}>
              <span style={{ fontSize:20 }}>{g.emoji}</span>
              <div style={{ flex:1, fontSize:13, color:"var(--t-text-muted,#b9b0d0)" }}>{g.title}</div>
              <button onClick={()=>onUpdate(g.id, { active:true })} style={{ ...S.btnSecondary, fontSize:11, padding:"3px 10px" }}>↺ Reactivar</button>
              <button onClick={()=>onDelete(g.id)} style={{ background:"none", border:"none", color:"var(--t-text-dim,#8f84ad)", cursor:"pointer", fontSize:18, lineHeight:1, padding:"0 2px" }}
