...
export function computeGoalHistory(goal, weeks) {
  ...
  if (goal.period === "weekly") {
    ...
    return Array.from({ length: 6 }, (_, i) => {
      ...
      return { label: MONTHS_SHORT[mo], count, met: isMax ? count <= goal.target : count >= goal.target, isPast: i < 5 };
    });
  } else {
    return Array.from({ length: 4 }, (_, i) => {
      ...
      return { label: String(yr), count, met: isMax ? count <= goal.target : count >= goal.target, isPast: i < 3 };
    });
  }
}
