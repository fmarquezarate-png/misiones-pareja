// ─────────────────────────────────────────────────────────────────────────────
// WeekStrip.jsx — v3.0.0
// Tira horizontal de los 7 días de la semana actual (L–D) con la fecha de hoy
// resaltada. Click en un día llama onSelectDay(dateStr).
// Se usa en Home y Semana — el usuario pidió que estos NO se pierdan.
//
// Props:
//   - week:        (opcional) info de la semana actual { start: Date, end: Date }
//   - missions:    array de misiones de la semana (para indicar densidad)
//   - onSelectDay: (dateStr) => void
//   - selected:    dateStr opcional para resaltar
// ─────────────────────────────────────────────────────────────────────────────
import { T } from "../styles.js";

const DOW = ["L","M","X","J","V","S","D"];

// Construye los 7 días de la semana ISO de today
function buildWeekDays(refDate = new Date()) {
  const d = new Date(refDate);
  d.setHours(0,0,0,0);
  const dow = (d.getDay() + 6) % 7; // L=0
