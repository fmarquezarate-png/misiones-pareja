// WeekTimeline.jsx — v3.0.0
// Vista timeline de la semana actual: misiones agrupadas por día,
// con riel vertical pink→purple→green y dots por día. Hoy se marca con
// dot pink + halo y label en pink.
//
// Props:
//   missions: [{ id, emoji, title, date (YYYY-MM-DD), time?, status, who, ... }]
//   today:    Date  (default new Date())
//   weekDays: [Date, Date, ...]   los 7 días lunes→domingo
//   renderCard: (mission) => ReactNode   (típicamente <MissionCard ... />)
//   showSinFecha: bool  (incluir agrupación "Sin fecha" al final)
...
export default function WeekTimeline({ missions, today = new Date(), weekDays, renderCard, showSinFecha = true }) {
  ...
  // Construir lista ordenada de días que mostraremos: todos los de la semana,
  // más cualquier día con misiones que caiga en la semana visible.
  ...
  return (
    <div style={{ position:"relative", paddingLeft:18, marginTop:6 }}>
      ...
      {visibleDays.map((dKey, i) => {
        ...
        const dowIdx = (dateObj.getDay() + 6) % 7; // Mon=0
